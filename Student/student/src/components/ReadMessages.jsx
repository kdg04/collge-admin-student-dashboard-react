...
const ReadMessages = ({ open, onClose }) => {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:8187/College/userMessages?userId=${userID}`); 
    ...
    <Draggable>
        ...
        >
            <Box sx={{ bgcolor: '#d6b4fc', width: '40%', p: 4, position: 'absolute', cursor: 'move', borderRadius:'12px', 
                      background: 'linear-gradient(to bottom, #d6b4fc, #ffd7eb)', // Linear gradient for modal background
                     }}>
                { messages.length > 0 ? (
                <>
                ...
                <Box style={{display:'flex', justifyContent:'space-between', margin:'5px'}}>
                  <Box style={{display:'flex', gap:'3px'}}>
                    <Button variant='contained' color='primary' disabled={currentMessageIndex === 0} onClick={handlePrevMessage}>Previous</Button>
                    <Button variant='contained' color='primary' disabled={currentMessageIndex === messages.length - 1} onClick={handleNextMessage}>Next</Button>                 
                  </Box>  
